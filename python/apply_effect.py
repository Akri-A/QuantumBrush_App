...
script_dir = Path(sys.path[0])         # Directory containing this script (python/)
app_path = script_dir.parent           # Project root
...
def process_effect(instr: dict):
    ...
    # Effects live alongside this script under python/<effect_id>/
    effect_path = script_dir / effect_id
    if not effect_path.is_dir():
        # Fallback: legacy layout at <app_root>/effect/<effect_id>/
    ...
    with Image.open(image_path) as img:
        req["stroke_input"]["image_rgba"] = np.asarray(img.convert("RGBA")).copy() # Ensure the image is in RGBA format
...
def apply_effect(req: dict):
    ...
    # Create diff mask: make unchanged pixels transparent so only the effect output is overlaid
    mask = np.all(new_image == input_image, axis=-1)
    new_image[mask] = [0, 0, 0, 0]  # Set unchanged pixels to transparent
