...
spec_steer = importlib.util.spec_from_file_location("steer", os.path.join(os.path.dirname(os.path.abspath(__file__)), "helper.py"))
...
def run(params):
    ...
    if len(clicks) < 2:
        raise ValueError(
            "Steerable requires at least 2 lasso selections: "
            "draw a closed loop around the source region, then another around the target region. "
            "Optionally draw a third for the paste region."
        )
    assert len(clicks) <= 3, "The number of clicks must be at most 3, i.e. source, target, paste"
    ...
    start_coordinates = []
    for click in clicks:
        matches = np.where((path == click).all(axis=1))[0]
        if len(matches) == 0:
            raise ValueError(
                f"Click at {click.tolist()} was not found in the drawn path. "
                "Make sure to use the Brush tool (not Dot tool) to draw each lasso region "
                "as a separate stroke."
            )
        start_coordinates.append(matches[0])
    ...
    if len(region_s) == 0:
        raise ValueError(
            "Source lasso region is empty. Draw a larger closed loop around the source area."
        )
    print("region_s============",len(region_s))
    region_t = utils.points_within_lasso(paths[1], border = (height, width))
    if len(region_t) == 0:
        raise ValueError(
            "Target lasso region is empty. Draw a larger closed loop around the target area."
        )
    ...
    if not params["user_input"].get("Source = Paste", params["user_input"].get("Souce=Paste", True)):
        if len(clicks) < 3:
            raise ValueError(
                "When 'Source = Paste' is unchecked, 3 lasso selections are required: "
                "source, target, and paste. Draw 3 separate closed loops."
            )
        if len(paths[2])>10:
    ...
    # Clip to valid [0, 1] range (pixels are in normalized space from SVD)
    new_pixels[:, :3] = np.clip(new_pixels[:, :3], 0.0, 1.0)
    new_pixels[:, 3] = np.clip(new_pixels[:, 3], 50.0 / 255.0, 1.0)
    ...
    if params["user_input"].get("Show source & target", params["user_input"].get("show source & target", False)):
