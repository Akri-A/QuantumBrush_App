...
def build_circuit(backend, params, source, target, n_qubits):
    ...
    # Souce and target state preparation — convert to JAX arrays
    initial_state = jnp.array(source, dtype=jnp.float32)
    target_state = jnp.array(target, dtype=jnp.float32)
    ...
    circuit = eqx.filter_jit(qml.qnode(dev, interface="jax")(partial(splitting_circuit, H_list=H_list, n_qubits=n_qubits)))
