...
def split_path_from_clicks(path,clicks):
    if len(clicks) == 0:
        return [interpolate_pixels(path)]
    ...
    # If no click indices were found (clicks not in path), find nearest points
    if len(click_indices) == 0:
        for click in clicks:
            dists = np.linalg.norm(np.array(path) - np.array(click), axis=1)
            click_indices.append(int(np.argmin(dists)))
        click_indices = sorted(set(click_indices))

    for idx, start in enumerate(click_indices):
        end = click_indices[idx + 1] if idx + 1 < len(click_indices) else len(path)
        segment = path[start:end]
        if len(segment) > 0:
            interp_path = interpolate_pixels(segment)
            split_paths.append(interp_path)

    # Fallback: if nothing was split, return the whole path
    if len(split_paths) == 0:
        split_paths = [interpolate_pixels(path)]
