...
def game_of_life(nhood):
    ...
    for qubit,state in enumerate(v):
        state = state / np.linalg.norm(state)  # Ensure exact normalization
...
def _downsample_region(copy_region, height, width):
    """Downsample a pixel region to at most MAX_REGION_PIXELS using a grid stride."""
    min_yx = copy_region.min(axis=0)
    max_yx = copy_region.max(axis=0)
    bbox_shape = max_yx - min_yx + 1

    mask = np.zeros(tuple(bbox_shape), dtype=bool)
    shifted = copy_region - min_yx
    mask[shifted[:, 0], shifted[:, 1]] = True

    scale = np.sqrt(len(copy_region) / MAX_REGION_PIXELS)
    stride = max(1, int(np.ceil(scale)))

    ds_mask = mask[::stride, ::stride]
    ys, xs = np.nonzero(ds_mask)
    ds_region = np.stack([ys * stride + min_yx[0], xs * stride + min_yx[1]], axis=1)
    ds_region = np.clip(ds_region, [0, 0], [height - 1, width - 1])
    return ds_region, stride
...
def run(params):
    ...
    if radius > 0:
        full_region = utils.points_within_radius(path, radius, border = (height, width))
    else:
        full_region = utils.points_within_lasso(path, border = (height, width))

    # Downsample the region if it's too large for per-pixel quantum simulation
    needs_interpolation = len(full_region) > MAX_REGION_PIXELS
    if needs_interpolation:
        print(f"Region has {len(full_region)} pixels, downsampling to ~{MAX_REGION_PIXELS} for quantum simulation")
        copy_region, stride = _downsample_region(full_region, height, width)
        print(f"Downsampled to {len(copy_region)} pixels (stride={stride})")
    else:
        copy_region = full_region

    def extract_neighbourhoods(x, y):
        neighbours = []
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ni, nj = (x + dx) % height, (y + dy) % width  # Wrap around edges
                neighbours.append((ni, nj))
        return neighbours

    n_iterations = params["user_input"]["Iterations"]
    for iteration in range(n_iterations):
        copy_selection = {}
        statevector_selection = {}
        semiclassical = {}
        after_iteration = {}
        total = len(copy_region)
        for idx, i in enumerate(copy_region):
            if idx % 500 == 0:
                print(f"Iteration {iteration+1}/{n_iterations}, pixel {idx}/{total}")
    ...
    # Interpolate results to the full region if we downsampled
    if needs_interpolation and len(after_iteration) > 0:
        print("Interpolating results to full region...")
        processed_points = np.array(list(after_iteration.keys()))
        processed_values = np.array(list(after_iteration.values()))
        full_values = griddata(processed_points, processed_values, full_region, method='nearest')
        after_iteration = {}
        for i, pixel in enumerate(full_region):
            after_iteration[(pixel[0], pixel[1])] = full_values[i]
