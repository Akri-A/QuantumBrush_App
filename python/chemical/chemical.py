...
def run(params):
    ...
    molecule = "H2"
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', molecule.lower() + '_parameters.json'), "r") as f:
        circuit_params = json.load(f)
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', molecule.lower() + '_circuit.qpy'), "rb") as f:
