...
def run(params):
    ...
    if len(clicks) < 2:
        raise ValueError("Clone brush requires at least 2 clicks: draw a lasso around the copy region, then click the paste destination.")
    # Use only the first 2 clicks if more were provided
    clicks = clicks[:2]
    ...
    # Remove any leftover points after the last click
    # Find the closest point in path to the last click
    dists = np.linalg.norm(path - clicks[-1], axis=1)
    cut_idx = int(np.argmin(dists))
    path = path[:cut_idx]  # Keep only the lasso portion before the paste click
    if len(path) < 3:
        raise ValueError("Not enough path points to form a lasso region. Draw a larger lasso before clicking the paste destination.")
