...
# Schrodinger
def schrodinger_rhs(psi, t, model):
...
term = ODETerm(schrodinger_rhs)
solver = Dopri5()
solution = diffeqsolve(term, solver, t0=0, t1=1, dt0=1/200, y0=initial_state, args=model)


# %%

# Numerical Integrator
def propagate(psi0, t0, t1, model, steps=200):
    ts = jnp.linspace(t0, t1, steps)
    sol = jax.experimental.ode.odeint(lambda y, t, p: schrodinger_rhs(y, t, p), psi0, ts, model)
    return ts, sol

@eqx.filter_jit
def loss_fn(model, inital_state, target_state, T=1.0, n_steps=40, C= 0):
    psi = propagate(inital_state, 0, T, model)
...
rho_f = propagate(initial_state, 0, T, model)
