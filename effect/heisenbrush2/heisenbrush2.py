...
def run_heisenberg_hardware(dt_list,saturation, lightness, radius):
    ...
    try:
        ...
        hsl = np.array([[h,lightness,  saturation] for h in normalized])

        return utils.hls_to_rgb(hsl)
...
def run(params):
    ...
    normalized_distances = [ strength * d / max(distances) for d in distances]
    ...
    for c,path in enumerate(split_paths):
        ...
        blended = (1 - strength) * image[x,y,:3] + strength * heisenberg_colors[c] * 255
